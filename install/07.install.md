...
```bash
git clone https://github.com/WalksvagenShtangencircule/SmartAccess.git /opt/rbt
cd /opt/rbt
git checkout main
php /opt/rbt/server/cli.php --version-local
...
1.  Get app
```bash
git clone https://github.com/WalksvagenShtangencircule/SmartAccess.git /opt/rbt && cd /opt/rbt
```
...
# Safe update (production)

For production updates use the script below; it creates a backup of local configs/customizations and runs post-update DB/index steps:
...
```bash
# Skip DB dump (if already done separately)
SKIP_DB_BACKUP=1 ./install/scripts/safe_update.sh main

# Skip composer install
SKIP_COMPOSER=1 ./install/scripts/safe_update.sh main
```
