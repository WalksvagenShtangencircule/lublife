...
*/
    ...
    openCameraListViewer: function (cameraId) {
        if (!modules.addresses.cameras.meta || !modules.addresses.cameras.meta.cameras) {
            return;
        }
        let cam = null;
        for (let i in modules.addresses.cameras.meta.cameras) {
            if (String(modules.addresses.cameras.meta.cameras[i].cameraId) === String(cameraId)) {
                cam = modules.addresses.cameras.meta.cameras[i];
                break;
            }
        }
        if (!cam) {
    ...
    route: function (params) {
        ...
        QUERY("cameras", "cameras", { by: (params.tree ? "tree" : false), query: (params.tree ? params.tree : false) }, true).
            ...
            cardTable({
                ...
                afterRedraw: () => {
                    modules.addresses.cameras.refreshCameraListThumbs();
                },
                edit: id => {
                ...
                rows: () => {
                    ...
                    for (let i in modules.addresses.cameras.meta.cameras) {
                        if (!params.id || params.id == modules.addresses.cameras.meta.cameras[i].cameraId) {
                            let c = modules.addresses.cameras.meta.cameras[i];
                            let cid = c.cameraId;
                            rows.push({
                                uid: cid,
                                cols: [
                                    ...
                                    {
                                        data: `<img class="rbt-entrance-cam-thumb" src="img/cctv.png" alt="" loading="lazy" data-camera-id="${cid}">`,
                                        click: cameraId => {
                                            modules.addresses.cameras.openCameraListViewer(cameraId);
                                        },
                                    ...
                                    {
                                        data: c.common ? i18n("addresses.yes") : i18n("addresses.no"),
                                        nowrap: true,
                                    },
                                    {
                                        data: modules.addresses.cameras.meta.models[c.model]?.title ?? "&nbsp;",
                                        nowrap: true,
                                    },
                                    {
                                        data: c.name ? c.name : "&nbsp;",
