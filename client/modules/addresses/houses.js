({
    ...
    _flatsTableColumnRegistry: {
        flatId: {
            ...
            cell: flat => ({ data: flat.flatId }),
        },
        floor: {
            title: () => i18n("addresses.floor"),
            cell: flat => ({ data: flat.floor ? flat.floor : "-" }),
        },
        flat: {
            title: () => i18n("addresses.flatColumnShort"),
            cell: flat => ({ data: flat.flat, nowrap: true, class: "rbt-flats-flat-cell" }),
        },
        contract: {
            title: () => i18n("addresses.contract"),
            cell: flat => ({ data: flat.contract ? flat.contract : "-", nowrap: true }),
        },
        login: {
            title: () => i18n("addresses.login"),
            cell: flat => ({ data: flat.login ? flat.login : "-", nowrap: true }),
        },
        openCode: {
        ...
        plog: {
            ...
            cell: flat => ({ data: flat.plog !== undefined && flat.plog !== null && String(flat.plog) !== "" ? String(flat.plog) : "-", nowrap: true }),
        },
        sipEnabled: {
            title: () => i18n("addresses.sipEnabled"),
            cell: flat => ({ data: parseInt(flat.sipEnabled) ? i18n("yes") : i18n("no"), nowrap: true }),
        },
        cmsEnabled: {
            title: () => i18n("addresses.cmsEnabled"),
            cell: flat => ({ data: parseInt(flat.cmsEnabled) ? i18n("yes") : i18n("no"), nowrap: true }),
        },
        cars: {
            title: () => i18n("addresses.cars"),
            cell: flat => {
                let s = flat.cars ? String(flat.cars) : "";
                return { data: s ? (s.length > 96 ? s.substring(0, 96) + "…" : s) : "-", nowrap: true };
            },
        },
        subscribersLimit: {
            title: () => i18n("addresses.subscribersLimit"),
            cell: flat => ({ data: (flat.subscribersLimit !== undefined && flat.subscribersLimit !== null && String(flat.subscribersLimit) !== "") ? flat.subscribersLimit : "-", nowrap: true }),
        },
        autoOpen: {
    ...
    getFlatsTableColumnPlan: function () {
        ...
        for (let i = 0; i < cfg.length; i++) {
            ...
            let visible = typeof item === "string" ? true : (item.visible !== false);
            if (!visible || !id || !reg[id]) {
                continue;
            }
    ...
    findCameraInHouseMeta: function (cameraId) {
        let cams = modules.addresses.houses.meta && modules.addresses.houses.meta.cameras;
        if (!cams || cams === false) {
            return null;
        }
        let id = String(cameraId);
        for (let i in cams) {
            if (String(cams[i].cameraId) === id) {
                return cams[i];
            }
        }
        return null;
    ...
    fillCameraStreamViewerModal: function (camera, modalTitle) {
        let dvrHttp = modules.addresses.houses.safeCameraWebUrl(camera.dvrStream);
        let liveHttp = modules.addresses.houses.safeCameraWebUrl(camera.stream);
        let dvrRtsp = modules.addresses.houses.rtspStreamUrl(camera.dvrStream);
        let liveRtsp = modules.addresses.houses.rtspStreamUrl(camera.stream);
        let embedUrl = dvrHttp || liveHttp;

        $("#tableModalLabel").text(modalTitle);

        let $body = $("#tableModalBody");
        $body.empty();

        let $toolbar = $('<div class="mb-2 d-flex flex-wrap align-items-center" style="gap:8px;"></div>');
        if (dvrHttp) {
            $toolbar.append($("<a>", {
                ...
                href: dvrHttp,
                text: i18n("addresses.openCameraMediaInTab"),
            }));
        }
        if (dvrRtsp && !dvrHttp) {
            $toolbar.append($("<a>", {
                class: "btn btn-sm btn-primary",
                href: dvrRtsp,
                text: i18n("addresses.openCameraRtspDvr"),
            }));
        }
        if (liveHttp && liveHttp !== dvrHttp) {
            $toolbar.append($("<a>", {
                class: "btn btn-sm " + (dvrHttp || dvrRtsp ? "btn-outline-secondary" : "btn-primary"),
                target: "_blank",
                rel: "noopener noreferrer",
                href: liveHttp,
                text: i18n("addresses.openLiveStreamInTab"),
            }));
        }
        if (liveRtsp && !liveHttp && liveRtsp !== dvrRtsp) {
            $toolbar.append($("<a>", {
                class: "btn btn-sm btn-outline-secondary",
                href: liveRtsp,
                text: i18n("addresses.openCameraRtspLive"),
            }));
        }
        ...
        if (embedUrl) {
            ...
            $body.append($("<p>", {
                class: "text-muted small mt-2 mb-0",
                text: i18n("addresses.rtspBrowserHint"),
            }));
        } else {
            $body.append($("<p>", { class: "text-muted", text: i18n("addresses.noCameraStreamInConfig") }));
            $body.append($("<p>", { class: "mt-2 mb-0" }).append($("<a>", {
                href: "?#addresses.cameras&id=" + encodeURIComponent(camera.cameraId),
                class: "nodec",
                text: i18n("addresses.camera"),
            })));
        }
    ...
    fillEntranceCameraModal: function (entrance, camera) {
        let et = i18n("addresses.entranceType" + entrance.entranceType.substring(0, 1).toUpperCase() + entrance.entranceType.substring(1) + "Full");
        if (!camera) {
            $("#tableModalLabel").text(et + " " + entrance.entrance);
            let $body = $("#tableModalBody");
            $body.empty();
            $body.append($("<p>", { class: "text-muted", text: i18n("addresses.cameraNotFoundById") }));
            $body.append($("<p>", { class: "mt-2 mb-0" }).append($("<a>", {
                href: "?#addresses.cameras&id=" + encodeURIComponent(entrance.cameraId),
                class: "nodec",
                text: i18n("addresses.camera"),
            })));
            $("#tableModal").off("hidden.bs.modal.rbtCamViewer").on("hidden.bs.modal.rbtCamViewer", function () {
        ...
        let title = et + " " + entrance.entrance;
        if (camera.name) {
            title += " — " + camera.name;
        }
        modules.addresses.houses.fillCameraStreamViewerModal(camera, title);
    },

    refreshEntranceCameraThumbs: function (houseId) {
        $("#rbt-house-entrances-" + houseId + " .rbt-entrance-cam-thumb[data-camera-id]").each(function () {
            let $img = $(this);
    ...
    refreshHouseCameraThumbs: function (houseId) {
        $("#rbt-house-cameras-" + houseId + " .rbt-entrance-cam-thumb[data-camera-id]").each(function () {
            let $img = $(this);
            let id = $img.attr("data-camera-id");
            if (!id) {
                return;
    ...
    openHouseCameraViewer: function (cameraId, houseId) {
        let cams = modules.addresses.houses.meta && modules.addresses.houses.meta.cameras;
        if (!cams || cams === false) {
            return;
        }
        let cam = null;
        for (let i in cams) {
            if (String(cams[i].cameraId) === String(cameraId)) {
                cam = cams[i];
                break;
            }
        }
        ...
        let title = cam.name ? (cam.name + " (#" + cam.cameraId + ")") : (i18n("addresses.camera") + " #" + cam.cameraId);
        modules.addresses.houses.fillCameraStreamViewerModal(cam, title);
    },

    openCameraStreamViewerByCameraId: function (cameraId, modalTitle) {
        if (!parseInt(cameraId, 10)) {
            return;
        }
        loadingStart();
        QUERY("cameras", "cameras", {
            ...
            query: cameraId,
                ...
                let cam = null;
                for (let j in list) {
                    if (String(list[j].cameraId) === String(cameraId)) {
                        cam = list[j];
                        break;
                    }
                }
                ...
                if (cam) {
                    modules.addresses.houses.fillCameraStreamViewerModal(cam, modalTitle);
                } else {
                    message(i18n("addresses.cameraNotFoundById"), i18n("addresses.camera"), 5);
                }
            }).
            fail(FAIL).
            always(loadingDone);
    ...
    openEntranceCameraViewer: function (entranceId, houseId) {
        let entrance = null;
        for (let i in modules.addresses.houses.meta.entrances) {
            if (modules.addresses.houses.meta.entrances[i].entranceId == entranceId) {
                entrance = modules.addresses.houses.meta.entrances[i];
        ...
        if (!entrance || !parseInt(entrance.cameraId, 10)) {
            return;
        }
        let camera = modules.addresses.houses.findCameraInHouseMeta(entrance.cameraId);
        if (camera) {
            modules.addresses.houses.fillEntranceCameraModal(entrance, camera);
            return;
        }
        loadingStart();
        QUERY("cameras", "cameras", {
            by: "id",
            query: entrance.cameraId,
        }, true).
            done(r => {
                let list = r.cameras && r.cameras.cameras ? r.cameras.cameras : [];
                let cam = null;
                for (let j in list) {
                    if (String(list[j].cameraId) === String(entrance.cameraId)) {
                        cam = list[j];
                        break;
                    }
                }
                ...
                modules.addresses.houses.fillEntranceCameraModal(entrance, cam);
            }).
            fail(FAIL).
            always(loadingDone);
    },
    ...
    renderHouse: function (houseId) {
        modules.addresses.houses.loadHouse(houseId, false, () => {
            cardTable({
                ...
                columns: (() => {
                    let plan = modules.addresses.houses.getFlatsTableColumnPlan();
                    let cols = [];
                    for (let c = 0; c < plan.length; c++) {
                        let col = {
                            title: plan[c].def.title(),
                            fullWidth: c === plan.length - 1,
                        };
                        if (plan[c].id === "flat") {
                            col.thClass = "rbt-flats-flat-col";
                        }
                        cols.push(col);
                    }
                    return cols;
            ...
            cardTable({
                target: "#altForm",
                id: "rbt-house-entrances-" + houseId,
                title: {
                ...
                afterRedraw: () => {
                    modules.addresses.houses.refreshEntranceCameraThumbs(houseId);
                },
                columns: [
                    {
                ...
                rows: () => {
                    ...
                    for (let i in modules.addresses.houses.meta.entrances) {
                        ...
                        let _ent = modules.addresses.houses.meta.entrances[i];
                        let _camId = _ent.cameraId;
                        let _hasCam = parseInt(_camId, 10) > 0;
                        rows.push({
                            uid: _ent.entranceId,
                            cols: [
                                _hasCam ? {
                                    data: `<img class="rbt-entrance-cam-thumb" src="img/cctv.png" alt="" loading="lazy" data-camera-id="${_camId}">`,
                                    click: entranceId => {
                                        modules.addresses.houses.openEntranceCameraViewer(entranceId, houseId);
                                    },
                                ...
                                {
                                    data: i18n("addresses.entranceType" + _ent.entranceType.substring(0, 1).toUpperCase() + _ent.entranceType.substring(1) + "Full"),
                                },
                                {
                                    data: _ent.entrance,
                                    nowrap: true,
                                ...
                                {
                                    data: parseInt(_ent.shared) ? i18n("yes") : i18n("no"),
                                },
                                {
                                    data: parseInt(_ent.shared) ? _ent.prefix : "-",
                                },
                            ],
                            dropDown: {
                                items: [
            ...
            if (modules.addresses.houses.meta.cameras === false) {
                ...
                cardTable({
                    ...
                    afterRedraw: () => {
                        modules.addresses.houses.refreshHouseCameraThumbs(houseId);
                    },
                    edit: mc,
                    columns: [
                        {
                    ...
                    rows: () => {
                        let rows = [];
                        for (let i in modules.addresses.houses.meta.cameras) {
                            let hc = modules.addresses.houses.meta.cameras[i];
                            let hid = hc.cameraId;
                            let hasCam = !!hid;
                            rows.push({
                                uid: hid || ("del-" + i),
                                cols: [
                                    hasCam ? {
                                        data: `<img class="rbt-entrance-cam-thumb" src="img/cctv.png" alt="" loading="lazy" data-camera-id="${hid}">`,
                                        click: cameraId => {
                                            modules.addresses.houses.openHouseCameraViewer(cameraId, houseId);
                                        },
                                        class: "rbt-entrance-preview-cell",
                                    } : {
                                        data: "—",
                                    ...
                                    },
                                    {
                                        data: hasCam
                                            ? (hc.url ? hc.url : "")
                                            : i18n("addresses.deleted"),
                                        click: hasCam ? ("#addresses.cameras&id=" + hid) : false,
                                    },
                                    {
                                        data: hc.name ? hc.name : "",
                                        nowrap: true,
                                    },
                                    {
                                        data: hc.comments ? hc.comments : "",
                                        nowrap: true,
                                    },
                                ],
                                dropDown: {
