({
            ...
            GET("addresses", "favorites", false, true).
                ...
                if (r && r.favorites) {
                    ...
                    for (let i in r.favorites) {
                        ...
                        h += `
                            <li class="nav-item address-favorite-row" style="margin-top: 3px;" title="${escapeHTML(r.favorites[i].title)}">
                                    <a href="${url}" class="nav-link address-favorite-link" onclick="xblur(); return true;">
                                ...
                                <button type="button" class="favorite-remove-btn" title="${escapeHTML(i18n("addresses.removeFavorite"))}" data-fav-object="${escapeHTML(r.favorites[i].object)}" data-fav-id="${escapeHTML(String(r.favorites[i].id))}" aria-label="${escapeHTML(i18n("addresses.removeFavorite"))}"><i class="fas fa-times" aria-hidden="true"></i></button>
                            </li>
                        `;
                    ...
                    let f = false;
                    let $favRows = $(h);
                    $favRows.find(".favorite-remove-btn").on("click", function (e) {
                        e.preventDefault();
                        e.stopPropagation();
                        let object = $(this).attr("data-fav-object");
                        let id = $(this).attr("data-fav-id");
                        modules.addresses.removeFavoriteFromMenu(object, id);
                    });
                    while (i.next().length) {
                        i = i.next();
                        if ($.trim(i.text()) == "") {
                            $favRows.insertBefore(i);
                            f = true;
                            break;
                        }
                    }
    ...
    toggleFavorite: function (object, id) {
        ...
        if (!f) {
            ...
            if (!title) {
                for (let j in modules.addresses.favorites) {
                    if (modules.addresses.favorites[j].object == object && modules.addresses.favorites[j].id == id) {
                        title = modules.addresses.favorites[j].title;
                        break;
                    }
                }
            }
            if (!title) {
                title = String(object) + " #" + String(id);
            }
    ...
    removeFavoriteFromMenu: function (object, id) {
        let title = "";
        for (let j in modules.addresses.favorites) {
            if (modules.addresses.favorites[j].object == object && String(modules.addresses.favorites[j].id) === String(id)) {
                title = modules.addresses.favorites[j].title;
                break;
            }
        }
    ...
    route: function (params) {
        ...
        document.title = i18n("windowTitle") + " :: " + i18n("moduleAddresses");
