 ...
 * @param {Function} [params.afterRedraw] - Вызывается после отрисовки tbody (первая отрисовка, смена страницы, фильтр).
...
function cardTable(params) {
    ...
    if (params.id) {
        h += `<table id= "${params.id}" class="table table-hover rbt-data-table">`;
    } else {
        h += `<table class="table table-hover rbt-data-table">`;
    }
    h += `<thead>`;
    ...
    for (let i in params.columns) {
        ...
        let _thc = params.columns[i].thClass ? ` class="${params.columns[i].thClass}"` : "";
        if (params.columns[i].fullWidth) {
            h += `<th nowrap style="width: 100%"${_thc}>${params.columns[i].title}</th>`;
        } else {
            h += `<th nowrap${_thc}>${params.columns[i].title}</th>`;
        }
        columns++;
    }
    ...
    function tbody(from, length) {
        ...
        for (let i = from; i < Math.min(rows.length, from + length); i++) {
            ...
            h += `>`;
            if (typeof params.edit === "function") {
                h += `<td class="hoverable pointer ${editClass} rbt-table-edit-cell" data-uid="${rows[i].uid}" title="${i18n("edit")}"><span class="rbt-table-action-chip rbt-table-action-chip--edit" role="presentation"><i class="fas fa-pen-to-square" aria-hidden="true"></i></span></td>`;
                w++;
            }
            ...
            if (rows[i].dropDown && hasDropDowns) {
                ...
                if (rows[i].dropDown.items.length === 1 && rows[i].dropDown.items[0].icon) {
                    ...
                    t += `<span class="pointer dropdown-toggle dropdown-toggle-no-icon rbt-table-menu-chip" id="${ddId}" data-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false">`;
                    if (rows[i].dropDown.icon) {
                        t += `<span class="rbt-table-action-chip rbt-table-action-chip--menu" role="presentation"><i class="fa-fw ${rows[i].dropDown.icon}" aria-hidden="true"></i></span>`;
                    } else {
                        t += `<span class="rbt-table-action-chip rbt-table-action-chip--menu" role="presentation"><i class="fas fa-ellipsis-vertical fa-fw" aria-hidden="true"></i></span>`;
                    }
                    t += `</span>`;
    ...
    function addHandlers() {
        ...
        if (typeof params.afterRedraw === "function") {
            params.afterRedraw();
        }
    }
